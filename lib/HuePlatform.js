...
const AccessoryFactory = require('./PowerOnSensors/Factory');
  ...
  _getPlatformAccessory(client, hueAccessory) {
    ...
    for (const cachedAccessory of this.cachedAccessories) {
      if (cachedAccessory.context.uniqueId === hueAccessory.uniqueId) {
        ...
        platformAccessory = this.accessoryFactory.createCachedPowerOnSensor(client, hueAccessory, cachedAccessory);
      }
    }
    ...
    if (platformAccessory === undefined) {
      platformAccessory = this.accessoryFactory.createPowerOnSensor(client, hueAccessory);
    }
