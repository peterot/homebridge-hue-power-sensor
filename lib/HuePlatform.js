...
class HuePlatform {
      ...
      client.lights.getAll()
        .then(lights => {
          for (const light of lights) {
            ...
            try {
              this.accessoryFactory.addSensorServicesToAccessory(accessory, client, light);

              if (light.name === "BH Sonos") {
                this.accessoryFactory.enableStateUpdates(accessory);
              }

              if (accessory.context.generic && this.config.enableGenericLights !== true
                && light.name !== "BH Sonos") {
                console.log("Generic light - ignoring");
                continue;
              }
  ...
  _getPlatformAccessory(client, hueAccessory) {
    ...
    this.accessoryFactory.linkHueAccessory(platformAccessory, client, hueAccessory);
